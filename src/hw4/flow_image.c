    ...
    for (int c = 0; c < im.c; c++){
        for (int x = 0; x < im.w; x++){
            for (int y = 0; y < im.h; y++){
                ...
                float left = (y == 0)? 0 : get_pixel(integ, x, y-1, c);
                float right = (x == 0)? 0 : get_pixel(integ, x-1, y, c);
                float opposite = (x == 0 || y == 0)? 0 : get_pixel(integ, x-1, y-1, c);
                value += left + right - opposite;
...
image box_filter_image(image im, int s)
{
    ...
    // return convolve_image(im, make_box_filter(s), 1);
    ...
    for (c = 0; c < S.c; c++){
        for (x = 0; x < S.w; x++){
            for (y = 0; y < S.h; y++ ){
                float value = get_pixel(integ, x+s, y+s, c);  
                float left = (y-s-1 < 0)? 0 : get_pixel(integ, x+s, y-s-1, c);
                float right = (x-s-1 < 0)? 0 : get_pixel(integ, x-s-1, y+s, c);
                float opposite = (x-s-1 < 0 || y-s-1 < 0)? 0 : get_pixel(integ, x-s-1, y-s-1, c);
                value += -left - right + opposite;

                if (x-s-1 < 0 || y-s-1 < 0 || x+s >= S.w || y+s >= S.h){
                    int wstart = (x+s >= S.w)? S.w-1 : x+s;
                    int hstart = (y+s >= S.h)? S.h-1 : y+s;
                    int wend = (x-s-1 < 0)? -1 : (x-s-1);
                    int hend = (y-s-1 < 0)? -1 : (y-s-1);
                    float trueBase = (wstart - wend) * (hstart - hend);
                    set_pixel(S, x, y, c, value/trueBase);
                } else {
                    set_pixel(S, x, y, c, value/base);
                }
            }
        }
    }
...
image velocity_image(image S, int stride)
{
    ...
    for(j = (stride-1)/2; j < S.h; j += stride){
        for(i = (stride-1)/2; i < S.w; i += stride){
            ...
            if (M_invert.data){
                matrix v = matrix_mult_matrix(M_invert, b);
                vx = v.data[0][0];
                vy = v.data[1][0];
            }
