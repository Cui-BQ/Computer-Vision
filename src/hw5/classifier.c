...
void update_layer(layer *l, double rate, double momentum, double decay)
{
    ...
    matrix m = axpy_matrix(-decay, l->w, l->dw);
    matrix delta_w_t = axpy_matrix(momentum, l->v, m);

    // save it to l->v
    l->v = delta_w_t;

    // Update l->w
    l->w = axpy_matrix(rate, delta_w_t, l->w);
...
// We want to know how our model performs on both train and test data, so we know the real performance of this model and if the model is overfitting or not.
//
...
// When rate=10, the loss is nan which means the model doesn't converge, and when rate is very small like 0.001, the accuracy isn't high as well. The model has the best accuracy rate when t=0.1. 
//
...
// The smaller weight decay the higher accuracy. 
//
...
// By using the neural_net model, the accuracy of LOGISTIC is 0.89, RELU is 0.923, LRELU is 0.92. RELU is a little better.
//
...
// To use RELU. When r=0.1 training accuracy = 0.956, test accuracy = 0.9479.
//              When r=0.01 train accuracy = 0.923, test accuract = 0.9244.
//              When r=0.001, train accuracy = 0.864, test accuracy = 0.8684.
// Thus, the best rate is 0.1
//
...
// The accuracy changes very small whit different decay (0.0001 to 0.1), but when decay=0.1, the modle has vest accuracy. A good decay rate can help with overfitting.
//
...
// When decay = 0.001, the accuracy is the highest. It's the best decay to reduce overfitting.
//
// 5.3.2.1 How well does your network perform on the CIFAR dataset?
// Isn't as good as on mnist. The accuracy is lower on CIFAR.
//
