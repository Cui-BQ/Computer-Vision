...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    for(j = 0; j < an; ++j){
        ...
        for (i = 0; i < bn; i++){
            descriptor db = b[i];
            float distance = l1_distance(da.data, db.data, da.n);
            if (distance < min){
                bind = i;
                min = distance;
            }
    ...
    *mn = count;
    free(seen);
    match *res = calloc(count, sizeof(match));
    memcpy(res, m, count * sizeof(match));
    return res;
}
...
point project_point(matrix H, point p)
{
    ...
    q.x = mult.data[0][0] / mult.data[2][0];
    q.y = mult.data[1][0] / mult.data[2][0];
    return q;
}
...
int model_inliers(matrix H, match *m, int n, float thresh)
{
    ...
    for (i = 0; i < n; i++){
        ...
        if (point_distance(Hp, m[i].q) < thresh){
            match tem = m[count];
            m[count] = m[i];
            m[i] = tem;
            count++;
...
void randomize_matches(match *m, int n)
{
    ...
    for (int i = 0; i < n; i++) { // for loop to shuffle
         int j = rand() % (i + 1); //randomise j for shuffle with Fisher Yates
         match tmp = m[j];
         m[j] = m[i];
         m[i] = tmp;
     }
...
matrix compute_homography(match *matches, int n)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        double data1[8] = {x, y, 1, 0, 0, 0, -x*xp, -y*xp};
        double data2[8] = {0, 0, 0, x, y, 1, -x*yp, -y*yp};
        memcpy(M.data[i*2], data1, 8*sizeof(double));
        memcpy(M.data[i*2+1], data2, 8*sizeof(double));
        b.data[i*2][0] = xp;
        b.data[i*2+1][0] = yp;
    }
    ...
    double data1[3] = {a.data[0][0], a.data[1][0], a.data[2][0]}; 
    double data2[3] = {a.data[3][0], a.data[4][0], a.data[5][0]}; 
    double data3[3] = {a.data[6][0], a.data[7][0],      1      };
    memcpy(H.data[0], data1, 3*sizeof(double));
    memcpy(H.data[1], data2, 3*sizeof(double));
    memcpy(H.data[2], data3, 3*sizeof(double));
    free_matrix(a);
    return H;
}
...
matrix RANSAC(match *m, int n, float thresh, int k, int cutoff)
{
    ...
    for (int i = 0; i < k; i++){
        ...
        int new = model_inliers(homography, m, n, thresh);
        if (new > best){
            best = new;
