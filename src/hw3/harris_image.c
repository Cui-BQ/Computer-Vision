...
image structure_matrix(image im, float sigma)
{
    ...
    for (int x = 0; x < S.w; x++){
        for (int y = 0; y < S.h; y++){
            float IxIx =  get_pixel(Ix, x, y, 0)*get_pixel(Ix, x, y, 0);
            float IyIy =  get_pixel(Iy, x, y, 0)*get_pixel(Iy, x, y, 0);
            float IxIy =  get_pixel(Ix, x, y, 0)*get_pixel(Iy, x, y, 0);
    ...
    return convolve_image(S, gaussian, 1);
}
...
image nms_image(image im, int w)
{
    ...
    for (int x = 0; x < im.w; x++){
        for (int y = 0; y < im.h; y++){
            float piexl = get_pixel(im, x, y, 0);
            for (int wx = -w; wx <= w; wx++){
                for (int wy = -w; wy <= w; wy++){
                    float neighbor_pixel = get_pixel(im, x+wx, y+wy, 0);
                    if (neighbor_pixel > piexl) {
...
descriptor *harris_corner_detector(image im, float sigma, float thresh, int nms, int *n)
{
    ...
    *n = count; // <- set *n equal to number of corners in image.
    ...
    for (int x = 0; x < Rnms.w; x++){
        for (int y = 0; y < Rnms.h; y++){
            if (get_pixel(Rnms, x, y, 0) >= thresh){
                d[count] = describe_index(im, y*Rnms.w + x);
                count += 1;
            }
        }
    }
